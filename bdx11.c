...
#define DELAYINMICROSECONDS 33125 /* mess around with this setting if song is off with the "video" */
...
void Init(void) {
	if ((WindowDisplay = XOpenDisplay(NULL)) == NULL) Die("Cannot open display\n");
...
void MainLoop() {	
	...
	if (Event.type == Expose) {
		for (unsigned i = 1; i < 6574; i++) {
			if (ReadFrame(i) != 0) Die("Missing frame '%u'\n", i);
			int PosX, PosY = 0;
			for (int j = 0; j < 24; j++) {
				PosX = 0;
				for (int k = 0; k < 64; k++) {
					if (Frame[j][k] == '1') XFillRectangle(WindowDisplay, WindowObj, DefaultGC(WindowDisplay, WindowScreen), PosX, PosY, PIXELWIDTH, PIXELHEIGHT);
					PosX += PIXELWIDTH;
				}
				PosY += PIXELHEIGHT;
